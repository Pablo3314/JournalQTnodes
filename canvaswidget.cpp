...
void CanvasWidget::renderStrokes(QPainter &p, const QRectF &visibleWorld)
{
    ...
    for (int idx : candidates) {
        ...
        const qreal zoomSafe = qMax<qreal>(m_zoom, 0.001);
        const qreal worldWidthPad = (s.width + 4.0) / zoomSafe;
        if (s.bounds.adjusted(-worldWidthPad, -worldWidthPad,
                              worldWidthPad, worldWidthPad).intersects(visibleWorld)) {
            drawStroke(s);
        }
    }
}
...
void CanvasWidget::paintEvent(QPaintEvent *)
{
    ...
    const qreal extraX = visibleWorld.width() * 2.0;
    const qreal extraY = visibleWorld.height() * 2.0;
    const QRectF paddedVisibleWorld = visibleWorld.adjusted(-extraX, -extraY, extraX, extraY);
